...
class PhoenixClick(mainActivity: MainActivity) : View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when (v?.id){
            R.id.img_phoenix -> {
                ...
                clickAgents = sharedPreferences.getFloat("clickPhoenix", 0F)
                pp()
                save()
                val a = "$click "
    ...
    private fun save(){
        ...
        editor.putFloat("clickPhoenix", clickAgents)
        editor.apply()
    }
