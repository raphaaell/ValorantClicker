...
class ReynaClick(mainActivity: MainActivity) : View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when (v?.id){
            R.id.img_reyna -> {
                ...
                clickAgents = sharedPreferences.getFloat("clickReyna", 0F)
                pp()
                save()
                val a = "$click "
    ...
    private fun save(){
        ...
        editor.putFloat("clickReyna", clickAgents)
        editor.apply()
    }
