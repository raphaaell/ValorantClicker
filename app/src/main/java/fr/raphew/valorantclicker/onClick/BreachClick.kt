...
class BreachClick(mainActivity: MainActivity) : View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when (v?.id){
            R.id.img_breach -> {
                ...
                clickAgents = sharedPreferences.getFloat("clickBreach", 0F)
                pp()
                save()
                val a = "$click "
    ...
    private fun save(){
        ...
        editor.putFloat("clickBreach", clickAgents)
        editor.apply()
    }
