...
class BrimstoneClick(mainActivity: MainActivity) : View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when (v?.id){
            R.id.img_brimstone -> {
                ...
                clickAgents = sharedPreferences.getFloat("clickBrimstone", 0F)
                pp()
                save()
                val a = "$click "
    ...
    private fun save(){
        ...
        editor.putFloat("clickBrimstone", clickAgents)
        editor.apply()
    }
