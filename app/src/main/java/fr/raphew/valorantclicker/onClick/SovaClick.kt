...
class SovaClick(mainActivity: MainActivity) : View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when (v?.id){
            R.id.img_sova -> {
                ...
                clickAgents = sharedPreferences.getFloat("clickSova", 0F)
                pp()
                save()
                val a = "$click "
    ...
    private fun save(){
        ...
        editor.putFloat("clickSova", clickAgents)
        editor.apply()
    }
